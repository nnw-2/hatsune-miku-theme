const vscode = require('vscode');
const fs = require('fs');
const path = require('path');
const packageJson = require('./package.json')

//REMINDER TO PUT FALSE FOR TRANSPARENCY GETTING OF VALUES OR UNDEFINED IS RETURNED AND EDITOR BREAKS

//ff will be the default transparency value if there isn't already a transparency value attatched by default.
const defaultValues = {
	"changeThemeEditor.changecolourWordHighlightBackground": "264f78",
	"changeThemeEditor.changetransparencyWordHighlightBackground" : "b5",

	"changeThemeEditor.changecolourWordHighlightBorder": "ff0000",
	"changeThemeEditor.changetransparencyWordHighlightBorder" : "00",

	"changeThemeEditor.changecolourLineHighlightBackground": "019ebb",
	"changeThemeEditor.changetransparencyLineHighlightBackground" : "5a",

	"changeThemeEditor.changecolourLineHighlightBorder": "4b4044",
	"changeThemeEditor.changetransparencyLineHighlightBorder" : "a1"
};

function getOrSetThemeValues(commandId , colourTrue , getOrSet="get" , updateValue=null) {
	const themeValuesPath = path.join(__dirname, 'themeValues.json');
	const themeValues = JSON.parse(fs.readFileSync(themeValuesPath , 'utf8'));

	if (getOrSet == "get") {
		return themeValues[colourTrue ? "colours" : "transparency"][commandId];
	}
	else {
		themeValues[colourTrue ? "colours" : "transparency"][commandId] = updateValue;
		fs.writeFileSync(themeValuesPath, JSON.stringify(themeValues, null, 2));
	}
}

function themeConfig(commandId , updateValue , colourTrue) {

	getOrSetThemeValues(commandId , colourTrue , "set" , updateValue) 

	const theme = {
	"name": "Hatsune Miku",
	"type": "dark",
	"semanticHighlighting": true,
	"semanticTokenColors": {
	"enumMember": {
	"foreground": "#019ebb"
	},
	"variable.constant": {
	"foreground": "#60a18b"
	},
	"variable.defaultLibrary": {
	"foreground": "#4b68ae"
	}
},
	"tokenColors": [
	{
	"name": "unison punctuation",
	"scope": "punctuation.definition.delayed.unison,punctuation.definition.list.begin.unison,punctuation.definition.list.end.unison,punctuation.definition.ability.begin.unison,punctuation.definition.ability.end.unison,punctuation.operator.assignment.as.unison,punctuation.separator.pipe.unison,punctuation.separator.delimiter.unison,punctuation.definition.hash.unison",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "haskell variable generic-type",
	"scope": "variable.other.generic-type.haskell",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "haskell storage type",
	"scope": "storage.type.haskell",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "support.variable.magic.python",
	"scope": "support.variable.magic.python",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "punctuation.separator.parameters.python",
	"scope": "punctuation.separator.period.python,punctuation.separator.element.python,punctuation.parenthesis.begin.python,punctuation.parenthesis.end.python",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "variable.parameter.function.language.special.self.python",
	"scope": "variable.parameter.function.language.special.self.python",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "storage.modifier.lifetime.rust",
	"scope": "storage.modifier.lifetime.rust",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "support.function.std.rust",
	"scope": "support.function.std.rust",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "entity.name.lifetime.rust",
	"scope": "entity.name.lifetime.rust",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "variable.language.rust",
	"scope": "variable.language.rust",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "support.constant.edge",
	"scope": "support.constant.edge",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "regexp constant character-class",
	"scope": "constant.other.character-class.regexp",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "regexp operator.quantifier",
	"scope": "keyword.operator.quantifier.regexp",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "punctuation.definition",
	"scope": "punctuation.definition.string.begin,punctuation.definition.string.end",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "Text",
	"scope": "variable.parameter.function",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Comment Markup Link",
	"scope": "comment markup.link",
	"settings": {
		"foreground": "#2b9493"
	}
	},
	{
	"name": "markup diff",
	"scope": "markup.changed.diff",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "diff",
	"scope": "meta.diff.header.from-file,meta.diff.header.to-file,punctuation.definition.from-file.diff,punctuation.definition.to-file.diff",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "inserted.diff",
	"scope": "markup.inserted.diff",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "deleted.diff",
	"scope": "markup.deleted.diff",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "c++ function",
	"scope": "meta.function.c,meta.function.cpp",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "c++ block",
	"scope": "punctuation.section.block.begin.bracket.curly.cpp,punctuation.section.block.end.bracket.curly.cpp,punctuation.terminator.statement.c,punctuation.section.block.begin.bracket.curly.c,punctuation.section.block.end.bracket.curly.c,punctuation.section.parens.begin.bracket.round.c,punctuation.section.parens.end.bracket.round.c,punctuation.section.parameters.begin.bracket.round.c,punctuation.section.parameters.end.bracket.round.c",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "js/ts punctuation separator key-value",
	"scope": "punctuation.separator.key-value",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "js/ts import keyword",
	"scope": "keyword.operator.expression.import",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "math js/ts",
	"scope": "support.constant.math",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "math property js/ts",
	"scope": "support.constant.property.math",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "js/ts variable.other.constant",
	"scope": "variable.other.constant",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "java type",
	"scope": [
		"storage.type.annotation.java",
		"storage.type.object.array.java"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "java source",
	"scope": "source.java",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "java modifier.import",
	"scope": "punctuation.section.block.begin.java,punctuation.section.block.end.java,punctuation.definition.method-parameters.begin.java,punctuation.definition.method-parameters.end.java,meta.method.identifier.java,punctuation.section.method.begin.java,punctuation.section.method.end.java,punctuation.terminator.java,punctuation.section.class.begin.java,punctuation.section.class.end.java,punctuation.section.inner-class.begin.java,punctuation.section.inner-class.end.java,meta.method-call.java,punctuation.section.class.begin.bracket.curly.java,punctuation.section.class.end.bracket.curly.java,punctuation.section.method.begin.bracket.curly.java,punctuation.section.method.end.bracket.curly.java,punctuation.separator.period.java,punctuation.bracket.angle.java,punctuation.definition.annotation.java,meta.method.body.java",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "java modifier.import",
	"scope": "meta.method.java",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "java modifier.import",
	"scope": "storage.modifier.import.java,storage.type.java,storage.type.generic.java",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "java instanceof",
	"scope": "keyword.operator.instanceof.java",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "java variable.name",
	"scope": "meta.definition.variable.name.java",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "operator logical",
	"scope": "keyword.operator.logical",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "operator bitwise",
	"scope": "keyword.operator.bitwise",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "operator channel",
	"scope": "keyword.operator.channel",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "support.constant.property-value.scss",
	"scope": "support.constant.property-value.scss,support.constant.property-value.css",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "CSS/SCSS/LESS Operators",
	"scope": "keyword.operator.css,keyword.operator.scss,keyword.operator.less",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "css color standard name",
	"scope": "support.constant.color.w3c-standard-color-name.css,support.constant.color.w3c-standard-color-name.scss",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "css comma",
	"scope": "punctuation.separator.list.comma.css",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "css attribute-name.id",
	"scope": "support.constant.color.w3c-standard-color-name.css",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "css property-name",
	"scope": "support.type.vendored.property-name.css",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "js/ts module",
	"scope": "support.module.node,support.type.object.module,support.module.node",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "entity.name.type.module",
	"scope": "entity.name.type.module",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "js variable readwrite",
	"scope": "variable.other.readwrite,meta.object-literal.key,support.variable.property,support.variable.object.process,support.variable.object.node",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "js/ts json",
	"scope": "support.constant.json",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "js/ts Keyword",
	"scope": [
		"keyword.operator.expression.instanceof",
		"keyword.operator.new",
		"keyword.operator.ternary",
		"keyword.operator.optional",
		"keyword.operator.expression.keyof"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "js/ts console",
	"scope": "support.type.object.console",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "js/ts support.variable.property.process",
	"scope": "support.variable.property.process",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "js console function",
	"scope": "entity.name.function,support.function.console",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "keyword.operator.misc.rust",
	"scope": "keyword.operator.misc.rust",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "keyword.operator.sigil.rust",
	"scope": "keyword.operator.sigil.rust",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "operator",
	"scope": "keyword.operator.delete",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "js dom",
	"scope": "support.type.object.dom",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "js dom variable",
	"scope": "support.variable.dom,support.variable.property.dom",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "keyword.operator",
	"scope": "keyword.operator.arithmetic,keyword.operator.comparison,keyword.operator.decrement,keyword.operator.increment,keyword.operator.relational",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "C operator assignment",
	"scope": "keyword.operator.assignment.c,keyword.operator.comparison.c,keyword.operator.c,keyword.operator.increment.c,keyword.operator.decrement.c,keyword.operator.bitwise.shift.c,keyword.operator.assignment.cpp,keyword.operator.comparison.cpp,keyword.operator.cpp,keyword.operator.increment.cpp,keyword.operator.decrement.cpp,keyword.operator.bitwise.shift.cpp",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Punctuation",
	"scope": "punctuation.separator.delimiter",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Other punctuation .c",
	"scope": "punctuation.separator.c,punctuation.separator.cpp",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "C type posix-reserved",
	"scope": "support.type.posix-reserved.c,support.type.posix-reserved.cpp",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "keyword.operator.sizeof.c",
	"scope": "keyword.operator.sizeof.c,keyword.operator.sizeof.cpp",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "python parameter",
	"scope": "variable.parameter.function.language.python",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "python type",
	"scope": "support.type.python",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "python logical",
	"scope": "keyword.operator.logical.python",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "pyCs",
	"scope": "variable.parameter.function.python",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "python block",
	"scope": "punctuation.definition.arguments.begin.python,punctuation.definition.arguments.end.python,punctuation.separator.arguments.python,punctuation.definition.list.begin.python,punctuation.definition.list.end.python",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "python function-call.generic",
	"scope": "meta.function-call.generic.python",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "python placeholder reset to normal string",
	"scope": "constant.character.format.placeholder.other.python",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Operators",
	"scope": "keyword.operator",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Compound Assignment Operators",
	"scope": "keyword.operator.assignment.compound",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Compound Assignment Operators js/ts",
	"scope": "keyword.operator.assignment.compound.js,keyword.operator.assignment.compound.ts",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Keywords",
	"scope": "keyword",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Namespaces",
	"scope": "entity.name.namespace",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Variables",
	"scope": "variable",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Variables",
	"scope": "variable.c",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Language variables",
	"scope": "variable.language",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Java Variables",
	"scope": "token.variable.parameter.java",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Java Imports",
	"scope": "import.storage.java",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Packages",
	"scope": "token.package.keyword",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Packages",
	"scope": "token.package",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Functions",
	"scope": [
		"entity.name.function",
		"meta.require",
		"support.function.any-method",
		"variable.function"
	],
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Classes",
	"scope": "entity.name.type.namespace",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Classes",
	"scope": "support.class, entity.name.type.class",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Class name",
	"scope": "entity.name.class.identifier.namespace.type",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Class name",
	"scope": [
		"entity.name.class",
		"variable.other.class.js",
		"variable.other.class.ts"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Class name php",
	"scope": "variable.other.class.php",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Type Name",
	"scope": "entity.name.type",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Keyword Control",
	"scope": "keyword.control",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Control Elements",
	"scope": "control.elements, keyword.operator.less",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Methods",
	"scope": "keyword.other.special-method",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Storage",
	"scope": "storage",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Storage JS TS",
	"scope": "token.storage",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Source Js Keyword Operator Delete,source Js Keyword Operator In,source Js Keyword Operator Of,source Js Keyword Operator Instanceof,source Js Keyword Operator New,source Js Keyword Operator Typeof,source Js Keyword Operator Void",
	"scope": "keyword.operator.expression.delete,keyword.operator.expression.in,keyword.operator.expression.of,keyword.operator.expression.instanceof,keyword.operator.new,keyword.operator.expression.typeof,keyword.operator.expression.void",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Java Storage",
	"scope": "token.storage.type.java",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Support",
	"scope": "support.function",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Support type",
	"scope": "support.type.property-name",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Support type",
	"scope": "support.constant.property-value",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Support type",
	"scope": "support.constant.font-name",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Meta tag",
	"scope": "meta.tag",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Strings",
	"scope": "string",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "Inherited Class",
	"scope": "entity.other.inherited-class",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Constant other symbol",
	"scope": "constant.other.symbol",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Integers",
	"scope": "constant.numeric",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Constants",
	"scope": "constant",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Constants",
	"scope": "punctuation.definition.constant",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Tags",
	"scope": "entity.name.tag",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Attributes",
	"scope": "entity.other.attribute-name",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Attribute IDs",
	"scope": "entity.other.attribute-name.id",
	"settings": {
		"fontStyle": "",
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Attribute class",
	"scope": "entity.other.attribute-name.class.css",
	"settings": {
		"fontStyle": "",
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Selector",
	"scope": "meta.selector",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Headings",
	"scope": "markup.heading",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Headings",
	"scope": "markup.heading punctuation.definition.heading, entity.name.section",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Units",
	"scope": "keyword.other.unit",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Bold",
	"scope": "markup.bold,todo.bold",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "Bold",
	"scope": "punctuation.definition.bold",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "markup Italic",
	"scope": "markup.italic, punctuation.definition.italic,todo.emphasis",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "emphasis md",
	"scope": "emphasis md",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown headings",
	"scope": "entity.name.section.markdown",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown heading Punctuation Definition",
	"scope": "punctuation.definition.heading.markdown",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "punctuation.definition.list.begin.markdown",
	"scope": "punctuation.definition.list.begin.markdown",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown heading setext",
	"scope": "markup.heading.setext",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Punctuation Definition Bold",
	"scope": "punctuation.definition.bold.markdown",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Inline Raw",
	"scope": "markup.inline.raw.markdown",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Inline Raw",
	"scope": "markup.inline.raw.string.markdown",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown List Punctuation Definition",
	"scope": "punctuation.definition.list.markdown",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Punctuation Definition String",
	"scope": [
		"punctuation.definition.string.begin.markdown",
		"punctuation.definition.string.end.markdown",
		"punctuation.definition.metadata.markdown"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "beginning.punctuation.definition.list.markdown",
	"scope": [
		"beginning.punctuation.definition.list.markdown"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Punctuation Definition Link",
	"scope": "punctuation.definition.metadata.markdown",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Underline Link/Image",
	"scope": "markup.underline.link.markdown,markup.underline.link.image.markdown",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Link Title/Description",
	"scope": "string.other.link.title.markdown,string.other.link.description.markdown",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Regular Expressions",
	"scope": "string.regexp",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "Escape Characters",
	"scope": "constant.character.escape",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Embedded",
	"scope": "punctuation.section.embedded, variable.interpolation",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Embedded",
	"scope": "punctuation.section.embedded.begin,punctuation.section.embedded.end",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "illegal",
	"scope": "invalid.illegal",
	"settings": {
		"foreground": "#ffffff"
	}
	},
	{
	"name": "illegal",
	"scope": "invalid.illegal.bad-ampersand.html",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Broken",
	"scope": "invalid.broken",
	"settings": {
		"foreground": "#ffffff"
	}
	},
	{
	"name": "Deprecated",
	"scope": "invalid.deprecated",
	"settings": {
		"foreground": "#ffffff"
	}
	},
	{
	"name": "Unimplemented",
	"scope": "invalid.unimplemented",
	"settings": {
		"foreground": "#ffffff"
	}
	},
	{
	"name": "Source Json Meta Structure Dictionary Json > String Quoted Json",
	"scope": "source.json meta.structure.dictionary.json > string.quoted.json",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Source Json Meta Structure Dictionary Json > String Quoted Json > Punctuation String",
	"scope": "source.json meta.structure.dictionary.json > string.quoted.json > punctuation.string",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Source Json Meta Structure Dictionary Json > Value Json > String Quoted Json,source Json Meta Structure Array Json > Value Json > String Quoted Json,source Json Meta Structure Dictionary Json > Value Json > String Quoted Json > Punctuation,source Json Meta Structure Array Json > Value Json > String Quoted Json > Punctuation",
	"scope": "source.json meta.structure.dictionary.json > value.json > string.quoted.json,source.json meta.structure.array.json > value.json > string.quoted.json,source.json meta.structure.dictionary.json > value.json > string.quoted.json > punctuation,source.json meta.structure.array.json > value.json > string.quoted.json > punctuation",
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "Source Json Meta Structure Dictionary Json > Constant Language Json,source Json Meta Structure Array Json > Constant Language Json",
	"scope": "source.json meta.structure.dictionary.json > constant.language.json,source.json meta.structure.array.json > constant.language.json",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] JSON Property Name",
	"scope": "support.type.property-name.json",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] JSON Punctuation for Property Name",
	"scope": "support.type.property-name.json punctuation",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "laravel blade tag",
	"scope": "text.html.laravel-blade source.php.embedded.line.html entity.name.tag.laravel-blade",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "laravel blade @",
	"scope": "text.html.laravel-blade source.php.embedded.line.html support.constant.laravel-blade",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "use statement for other classes",
	"scope": "support.other.namespace.use.php,support.other.namespace.use-as.php,support.other.namespace.php,entity.other.alias.php,meta.interface.php",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "error suppression",
	"scope": "keyword.operator.error-control.php",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "php instanceof",
	"scope": "keyword.operator.type.php",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "style double quoted array index normal begin",
	"scope": "punctuation.section.array.begin.php",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "style double quoted array index normal end",
	"scope": "punctuation.section.array.end.php",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "php illegal.non-null-typehinted",
	"scope": "invalid.illegal.non-null-typehinted.php",
	"settings": {
		"foreground": "#f44747"
	}
	},
	{
	"name": "php types",
	"scope": "storage.type.php,meta.other.type.phpdoc.php,keyword.other.type.php,keyword.other.array.phpdoc.php",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "php call-function",
	"scope": "meta.function-call.php,meta.function-call.object.php,meta.function-call.static.php",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "php function-resets",
	"scope": "punctuation.definition.parameters.begin.bracket.round.php,punctuation.definition.parameters.end.bracket.round.php,punctuation.separator.delimiter.php,punctuation.section.scope.begin.php,punctuation.section.scope.end.php,punctuation.terminator.expression.php,punctuation.definition.arguments.begin.bracket.round.php,punctuation.definition.arguments.end.bracket.round.php,punctuation.definition.storage-type.begin.bracket.round.php,punctuation.definition.storage-type.end.bracket.round.php,punctuation.definition.array.begin.bracket.round.php,punctuation.definition.array.end.bracket.round.php,punctuation.definition.begin.bracket.round.php,punctuation.definition.end.bracket.round.php,punctuation.definition.begin.bracket.curly.php,punctuation.definition.end.bracket.curly.php,punctuation.definition.section.switch-block.end.bracket.curly.php,punctuation.definition.section.switch-block.start.bracket.curly.php,punctuation.definition.section.switch-block.begin.bracket.curly.php,punctuation.definition.section.switch-block.end.bracket.curly.php",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "support php constants",
	"scope": "support.constant.core.rust",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "support php constants",
	"scope": "support.constant.ext.php,support.constant.std.php,support.constant.core.php,support.constant.parser-token.php",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "php goto",
	"scope": "entity.name.goto-label.php,support.other.php",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "php logical/bitwise operator",
	"scope": "keyword.operator.logical.php,keyword.operator.bitwise.php,keyword.operator.arithmetic.php",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "php regexp operator",
	"scope": "keyword.operator.regexp.php",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "php comparison",
	"scope": "keyword.operator.comparison.php",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "php heredoc/nowdoc",
	"scope": "keyword.operator.heredoc.php,keyword.operator.nowdoc.php",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "python function decorator @",
	"scope": "meta.function.decorator.python",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "python function support",
	"scope": "support.token.decorator.python,meta.function.decorator.identifier.python",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "parameter function js/ts",
	"scope": "function.parameter",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "brace function",
	"scope": "function.brace",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "parameter function ruby cs",
	"scope": "function.parameter.ruby, function.parameter.cs",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "constant.language.symbol.ruby",
	"scope": "constant.language.symbol.ruby",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "rgb-value",
	"scope": "rgb-value",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "rgb value",
	"scope": "inline-color-decoration rgb-value",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "rgb value less",
	"scope": "less rgb-value",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "sass selector",
	"scope": "selector.sass",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "ts primitive/builtin types",
	"scope": "support.type.primitive.ts,support.type.builtin.ts,support.type.primitive.tsx,support.type.builtin.tsx",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "block scope",
	"scope": "block.scope.end,block.scope.begin",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "cs storage type",
	"scope": "storage.type.cs",
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "cs local variable",
	"scope": "entity.name.variable.local.cs",
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"scope": "token.info-token",
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"scope": "token.warn-token",
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"scope": "token.error-token",
	"settings": {
		"foreground": "#f44747"
	}
	},
	{
	"scope": "token.debug-token",
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "String interpolation",
	"scope": [
		"punctuation.definition.template-expression.begin",
		"punctuation.definition.template-expression.end",
		"punctuation.section.embedded"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Reset JavaScript string interpolation expression",
	"scope": [
		"meta.template.expression"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Import module JS",
	"scope": [
		"keyword.operator.module"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "js Flowtype",
	"scope": [
		"support.type.type.flowtype"
	],
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "js Flow",
	"scope": [
		"support.type.primitive"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "js class prop",
	"scope": [
		"meta.property.object"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "js func parameter",
	"scope": [
		"variable.parameter.function.js"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "js template literals begin",
	"scope": [
		"keyword.other.template.begin"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "js template literals end",
	"scope": [
		"keyword.other.template.end"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "js template literals variable braces begin",
	"scope": [
		"keyword.other.substitution.begin"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "js template literals variable braces end",
	"scope": [
		"keyword.other.substitution.end"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "js operator.assignment",
	"scope": [
		"keyword.operator.assignment"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "go operator",
	"scope": [
		"keyword.operator.assignment.go"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "go operator",
	"scope": [
		"keyword.operator.arithmetic.go",
		"keyword.operator.address.go"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "Go package name",
	"scope": [
		"entity.name.package.go"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "elm prelude",
	"scope": [
		"support.type.prelude.elm"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "elm constant",
	"scope": [
		"support.constant.elm"
	],
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "template literal",
	"scope": [
		"punctuation.quasi.element"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "html/pug (jade) escaped characters and entities",
	"scope": [
		"constant.character.entity"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "styling css pseudo-elements/classes to be able to differentiate from classes which are the same colour",
	"scope": [
		"entity.other.attribute-name.pseudo-element",
		"entity.other.attribute-name.pseudo-class"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Clojure globals",
	"scope": [
		"entity.global.clojure"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Clojure symbols",
	"scope": [
		"meta.symbol.clojure"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Clojure constants",
	"scope": [
		"constant.keyword.clojure"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "CoffeeScript Function Argument",
	"scope": [
		"meta.arguments.coffee",
		"variable.parameter.function.coffee"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Ini Default Text",
	"scope": [
		"source.ini"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "Makefile prerequisities",
	"scope": [
		"meta.scope.prerequisites.makefile"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Makefile text colour",
	"scope": [
		"source.makefile"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Groovy import names",
	"scope": [
		"storage.modifier.import.groovy"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Groovy Methods",
	"scope": [
		"meta.method.groovy"
	],
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "Groovy Variables",
	"scope": [
		"meta.definition.variable.name.groovy"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "Groovy Inheritance",
	"scope": [
		"meta.definition.class.inherited.classes.groovy"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "HLSL Semantic",
	"scope": [
		"support.variable.semantic.hlsl"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "HLSL Types",
	"scope": [
		"support.type.texture.hlsl",
		"support.type.sampler.hlsl",
		"support.type.object.hlsl",
		"support.type.object.rw.hlsl",
		"support.type.fx.hlsl",
		"support.type.object.hlsl"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "SQL Variables",
	"scope": [
		"text.variable",
		"text.bracketed"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "types",
	"scope": [
		"support.type.swift",
		"support.type.vb.asp"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "heading 1, keyword",
	"scope": [
		"entity.name.function.xi"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "heading 2, callable",
	"scope": [
		"entity.name.class.xi"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "heading 3, property",
	"scope": [
		"constant.character.character-class.regexp.xi"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "heading 4, type, class, interface",
	"scope": [
		"constant.regexp.xi"
	],
	"settings": {
		"foreground": "#b94f61"
	}
	},
	{
	"name": "heading 5, enums, preprocessor, constant, decorator",
	"scope": [
		"keyword.control.xi"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "heading 6, number",
	"scope": [
		"invalid.xi"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "string",
	"scope": [
		"beginning.punctuation.definition.quote.markdown.xi"
	],
	"settings": {
		"foreground": "#198764"
	}
	},
	{
	"name": "comments",
	"scope": [
		"beginning.punctuation.definition.list.markdown.xi"
	],
	"settings": {
		"foreground": "#2b9493"
	}
	},
	{
	"name": "link",
	"scope": [
		"constant.character.xi"
	],
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "accent",
	"scope": [
		"accent.xi"
	],
	"settings": {
		"foreground": "#67a8ff"
	}
	},
	{
	"name": "wikiword",
	"scope": [
		"wikiword.xi"
	],
	"settings": {
		"foreground": "#60a18b"
	}
	},
	{
	"name": "language operators like '+', '-' etc",
	"scope": [
		"constant.other.color.rgb-value.xi"
	],
	"settings": {
		"foreground": "#ffffff"
	}
	},
	{
	"name": "elements to dim",
	"scope": [
		"punctuation.definition.tag.xi"
	],
	"settings": {
		"foreground": "#2b9493"
	}
	},
	{
	"name": "C++/C#",
	"scope": [
		"entity.name.label.cs",
		"entity.name.scope-resolution.function.call",
		"entity.name.scope-resolution.function.definition"
	],
	"settings": {
		"foreground": "#4b68ae"
	}
	},
	{
	"name": "Markdown underscore-style headers",
	"scope": [
		"entity.name.label.cs",
		"markup.heading.setext.1.markdown",
		"markup.heading.setext.2.markdown"
	],
	"settings": {
		"foreground": "#29ccd0"
	}
	},
	{
	"name": "meta.brace.square",
	"scope": [
		" meta.brace.square"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "Comments",
	"scope": "comment, punctuation.definition.comment",
	"settings": {
		"fontStyle": "italic",
		"foreground": "#2b9493"
	}
	},
	{
	"name": "[VSCODE-CUSTOM] Markdown Quote",
	"scope": "markup.quote.markdown",
	"settings": {
		"foreground": "#2b9493"
	}
	},
	{
	"name": "punctuation.definition.block.sequence.item.yaml",
	"scope": "punctuation.definition.block.sequence.item.yaml",
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"scope": [
		"constant.language.symbol.elixir"
	],
	"settings": {
		"foreground": "#019ebb"
	}
	},
	{
	"name": "js/ts italic",
	"scope": "entity.other.attribute-name.js,entity.other.attribute-name.ts,entity.other.attribute-name.jsx,entity.other.attribute-name.tsx,variable.parameter,variable.language.super",
	"settings": {
		"fontStyle": "italic"
	}
	},
	{
	"name": "comment",
	"scope": "comment.line.double-slash,comment.block.documentation",
	"settings": {
		"fontStyle": "italic"
	}
	},
	{
	"name": "Python Keyword Control",
	"scope": "keyword.control.import.python,keyword.control.flow.python",
	"settings": {
		"fontStyle": "italic"
	}
	},
	{
	"name": "markup.italic.markdown",
	"scope": "markup.italic.markdown",
	"settings": {
		"fontStyle": "italic"
	}
	}
],
	"colors": {
	"foreground": "#29ccd0",
	"focusBorder": "#29ccd0",
	"selection.background": "#257985cd",
	"scrollbar.shadow": "#29ccd0",
	"activityBar.foreground": "#29ccd0",
	"activityBar.background": "#4b4044",
	"activityBar.inactiveForeground": "#29ccd0",
	"activityBarBadge.foreground": "#df1161",
	"activityBarBadge.background": "#303137",
	"activityBar.border": "#29ccd0",
	"activityBar.activeBackground": "#303030",
	"sideBar.background": "#2c2d35",
	"sideBar.foreground": "#87cac9",
	"sideBarSectionHeader.background": "#1a1a1e",
	"sideBarSectionHeader.foreground": "#29ccd0",
	"sideBarSectionHeader.border": "#df1161ab",
	"sideBarTitle.foreground": "#29ccd0",
	"sideBar.border": "#29ccd0",
	"list.inactiveSelectionBackground": "#212126",
	"list.inactiveSelectionForeground": "#29ccd0",
	"list.hoverBackground": "#178c9c",
	"list.hoverForeground": "#fdf6eb",
	"list.activeSelectionBackground": "#1a1a1e",
	"list.activeSelectionForeground": "#29ccd0",
	"tree.indentGuidesStroke": "#1e8594",
	"list.dropBackground": "#395a63",
	"list.highlightForeground": "#29ccd0",
	"list.focusBackground": "#2c2d35",
	"list.focusForeground": "#fdf6eb",
	"listFilterWidget.background": "#242424",
	"listFilterWidget.outline": "#29cccd",
	"listFilterWidget.noMatchesOutline": "#b6436e",
	"statusBar.foreground": "#29ccd0",
	"statusBar.background": "#4b4044",
	"statusBarItem.hoverBackground": "#303030",
	"statusBar.border": "#29ccd0",
	"statusBar.debuggingBackground": "#15434c",
	"statusBar.debuggingForeground": "#c9386f",
	"statusBar.debuggingBorder": "#29ccd0",
	"statusBar.noFolderBackground": "#2b2d35",
	"statusBar.noFolderForeground": "#29ccd0",
	"statusBar.noFolderBorder": "#29ccd0",
	"statusBarItem.remoteBackground": "#29ccd0c7",
	"statusBarItem.remoteForeground": "#bd2156",
	"titleBar.activeBackground": "#2c2d35",
	"titleBar.activeForeground": "#29ccd0",
	"titleBar.inactiveBackground": "#6d6564",
	"titleBar.inactiveForeground": "#c9e3e2",
	"titleBar.border": "#2ecfd4",
	"menubar.selectionForeground": "#79e3e7",
	"menubar.selectionBackground": "#303030",
	"menubar.selectionBorder": "#29ccd0",
	"menu.foreground": "#29ccd0",
	"menu.background": "#31333a",
	"menu.selectionForeground": "#fdf6eb",
	"menu.selectionBackground": "#b0b3d8",
	"menu.selectionBorder": "#e90a50a1",
	"menu.separatorBackground": "#218aa1",
	"menu.border": "#212027",
	"button.background": "#288391",
	"button.foreground": "#fdf6eb",
	"button.hoverBackground": "#86c9c8",
	"button.secondaryForeground": "#fdf6eb",
	"button.secondaryBackground": "#288391",
	"button.secondaryHoverBackground": "#86c9c8",
	"input.background": "#2c2d35",
	"input.border": "#29ccd0",
	"input.foreground": "#fdf6eb",
	"inputOption.activeBackground": "#29ccd09d",
	"inputOption.activeBorder": "#d91561",
	"inputOption.activeForeground": "#fdf6eb",
	"input.placeholderForeground": "#fbfdff",
	"textLink.foreground": "#df0f60",
	"editor.background": "#20272f", 
	"editor.foreground": "#019ebb",
	"editorLineNumber.foreground": "#019ebb",
	"editorCursor.foreground": "#95d3d2",
	"editorCursor.background": "#131517",
	"editor.selectionBackground": "#264f78",
	"editor.inactiveSelectionBackground": "#3a3d41fe",
	"editorWhitespace.foreground": "#e3e4e229",
	"editor.selectionHighlightBackground": "#add6ff26",
	"editor.selectionHighlightBorder": "#495F77",
	"editor.findMatchBackground": "#019ebbd7",
	"editor.findMatchBorder": "#df1161ef",
	"editor.findMatchHighlightBackground": "#29ccd050",
	"editor.findMatchHighlightBorder": "#ffffff00",
	"editor.findRangeHighlightBackground": "#264f7856",
	"editor.findRangeHighlightBorder": "#ffffff00",
	"editor.rangeHighlightBackground": "#fdf6eb16",
	"editor.rangeHighlightBorder": "#ffffff00",
	"editor.hoverHighlightBackground": "#29ccd044",
	"editor.wordHighlightStrongBackground": "#" + getOrSetThemeValues("changeThemeEditor.changecolourWordHighlightBackground",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyWordHighlightBackground",false),
	"editor.wordHighlightStrongBorder": "#" + getOrSetThemeValues("changeThemeEditor.changecolourWordHighlightBorder",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyWordHighlightBorder",false),
	"editor.wordHighlightBackground": "#" + getOrSetThemeValues("changeThemeEditor.changecolourWordHighlightBackground",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyWordHighlightBackground",false),
	"editor.wordHighlightBorder": "#" + getOrSetThemeValues("changeThemeEditor.changecolourWordHighlightBorder",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyWordHighlightBorder",false),
	"editor.lineHighlightBackground": "#" + getOrSetThemeValues("changeThemeEditor.changecolourLineHighlightBackground",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyLineHighlightBackground",false),
	"editor.lineHighlightBorder": "#" + getOrSetThemeValues("changeThemeEditor.changecolourLineHighlightBorder",true) + getOrSetThemeValues("changeThemeEditor.changetransparencyLineHighlightBorder",false),
	"editorLineNumber.activeForeground": "#00f9ff",
	"editorLink.activeForeground": "#df0f60",
	"editorIndentGuide.background1": "#29ccd09f",
	"editorIndentGuide.activeBackground1": "#00f9ff",
	"editorRuler.foreground": "#264f78",
	"editorBracketMatch.background": "#00f9ff7c", //changes the background of the matching brackets
	"editorBracketMatch.border": "#4b4044",       //changes the border of the matching brackets
	"editor.foldBackground": "#fdf6eb16",
	"editorOverviewRuler.background": "#264f78bf",
	"editorOverviewRuler.border": "#df116180",
	"editorError.foreground": "#f2064c",
	"editorError.background": "#b73b3400",
	"editorError.border": "#ffffff00",
	"editorWarning.foreground": "#e18b81",
	"editorWarning.background": "#A9904000",
	"editorWarning.border": "#ffffff00",
	"editorInfo.foreground": "#0c92a5",
	"editorInfo.background": "#4490BF00",
	"editorInfo.border": "#4490BF00",
	"editorGutter.background": "#202020",
	"editorGutter.modifiedBackground": "#019ebb",
	"editorGutter.addedBackground": "#bbc1c6",
	"editorGutter.deletedBackground": "#df0f60",
	"editorGutter.foldingControlForeground": "#fdf6eb",
	"editorCodeLens.foreground": "#fdf6eb95",
	"editorGroup.border": "#29ccd0",
	"diffEditor.insertedTextBackground": "#9bb95538",
	"diffEditor.insertedTextBorder": "#ff000000",
	"diffEditor.removedTextBackground": "#ff000033",
	"diffEditor.border": "#29ccd0",
	"panel.background": "#212126",
	"panel.border": "#29ccd0",
	"panelTitle.activeBorder": "#29ccd0",
	"panelTitle.activeForeground": "#178c9c",
	"panelTitle.inactiveForeground": "#29ccd0",
	"badge.background": "#303137",
	"badge.foreground": "#df1161",
	"terminal.foreground": "#29ccd0",
	"terminal.selectionBackground": "#6e656412",
	"terminalCursor.background": "#20808e",
	"terminalCursor.foreground": "#fdf6eb",
	"terminal.border": "#29ccd0a1",
	"terminal.ansiBlack": "#000000",
	"terminal.ansiBlue": "#2472c8",
	"terminal.ansiBrightBlack": "#666666",
	"terminal.ansiBrightBlue": "#3b8eea",
	"terminal.ansiBrightCyan": "#29b8db",
	"terminal.ansiBrightGreen": "#23d18b",
	"terminal.ansiBrightMagenta": "#d670d6",
	"terminal.ansiBrightRed": "#f14c4c",
	"terminal.ansiBrightWhite": "#e5e5e5",
	"terminal.ansiBrightYellow": "#f5f543",
	"terminal.ansiCyan": "#11a8cd",
	"terminal.ansiGreen": "#0dbc79",
	"terminal.ansiMagenta": "#bc3fbc",
	"terminal.ansiRed": "#cd3131",
	"terminal.ansiWhite": "#e5e5e5",
	"terminal.ansiYellow": "#e5e510",
	"breadcrumb.background": "#3e3e44",
	"breadcrumb.foreground": "#29ccd0",
	"breadcrumb.focusForeground": "#d82a6d",
	"editorGroupHeader.border": "#29ccd0",
	"editorGroupHeader.tabsBackground": "#2c2d35",
	"editorGroupHeader.tabsBorder": "#0398ac",
	"tab.activeForeground": "#29ccd0",
	"tab.border": "#33bfc4",
	"tab.activeBackground": "#202020",
	"tab.activeBorder": "#e20e61",
	"tab.activeBorderTop": "#29ccd0",
	"tab.inactiveBackground": "#303030",
	"tab.inactiveForeground": "#33bfc4db",
	"tab.hoverBackground": "#151515d1",
	"tab.hoverForeground": "#29ccd0",
	"tab.hoverBorder": "#fbfdff",
	"scrollbarSlider.background": "#b9e5e46e",
	"scrollbarSlider.hoverBackground": "#b9e5e4cb",
	"scrollbarSlider.activeBackground": "#b9e5e4e9",
	"progressBar.background": "#29ccd0",
	"widget.shadow": "#178c9c",
	"editorWidget.foreground": "#fdf6eb",
	"editorWidget.background": "#15434c",
	"editorWidget.resizeBorder": "#0496aa",
	"pickerGroup.border": "#0496aa",
	"pickerGroup.foreground": "#2aced2",
	"debugToolBar.background": "#303030",
	"debugToolBar.border": "#29ccd0",
	"notifications.foreground": "#29ccd0",
	"notifications.background": "#2f3139",
	"notificationToast.border": "#29ccd058",
	"notificationsErrorIcon.foreground": "#f2064c",
	"notificationsWarningIcon.foreground": "#e18b81",
	"notificationsInfoIcon.foreground": "#0c92a5",
	"notificationCenter.border": "#29ccd058",
	"notificationCenterHeader.foreground": "#7ddce2",
	"notificationCenterHeader.background": "#1c1d24",
	"notifications.border": "#c9e2e2",
	"gitDecoration.addedResourceForeground": "#bbc1c6",
	"gitDecoration.conflictingResourceForeground": "#b94f61",
	"gitDecoration.deletedResourceForeground": "#df0f60",
	"gitDecoration.ignoredResourceForeground": "#2b9493",
	"gitDecoration.modifiedResourceForeground": "#019ebb",
	"gitDecoration.stageDeletedResourceForeground": "#df0f60",
	"gitDecoration.stageModifiedResourceForeground": "#019ebb",
	"gitDecoration.submoduleResourceForeground": "#73c6c1",
	"gitDecoration.untrackedResourceForeground": "#5ba7b2",
	"editorMarkerNavigation.background": "#2c2d35",
	"editorMarkerNavigationError.background": "#f2064c",
	"editorMarkerNavigationWarning.background": "#e18b81",
	"editorMarkerNavigationInfo.background": "#0c92a5",
	"merge.currentHeaderBackground": "#367366fe",
	"merge.currentContentBackground": "#27403Bfe",
	"merge.incomingHeaderBackground": "#395F8Ffe",
	"merge.incomingContentBackground": "#28384Bfe",
	"merge.commonHeaderBackground": "#383838fe",
	"merge.commonContentBackground": "#282828fe",
	"editorSuggestWidget.background": "#2c2d35",
	"editorSuggestWidget.border": "#019ebb",
	"editorSuggestWidget.foreground": "#fbfdff",
	"editorSuggestWidget.highlightForeground": "#29ccd0",
	"editorSuggestWidget.selectedBackground": "#4b4044",
	"editorHoverWidget.foreground": "#fbfdff",
	"editorHoverWidget.background": "#2c2d35",
	"editorHoverWidget.border": "#29ccd0",
	"peekView.border": "#29ccd0",
	"peekViewEditor.background": "#303030",
	"peekViewEditorGutter.background": "#202020",
	"peekViewEditor.matchHighlightBackground": "#264f78b7",
	"peekViewEditor.matchHighlightBorder": "#202020bb",
	"peekViewResult.background": "#2c2d35",
	"peekViewResult.fileForeground": "#29ccd0",
	"peekViewResult.lineForeground": "#fdf6eb",
	"peekViewResult.matchHighlightBackground": "#019ebbb1",
	"peekViewResult.selectionBackground": "#178c9c87",
	"peekViewResult.selectionForeground": "#fdf6eb",
	"peekViewTitle.background": "#2c2d35",
	"peekViewTitleDescription.foreground": "#29ccd0",
	"peekViewTitleLabel.foreground": "#29ccd0",
	"icon.foreground": "#29ccd0",
	"checkbox.background": "#2c2d35",
	"checkbox.foreground": "#fdf6eb",
	"checkbox.border": "#29ccd0",
	"dropdown.background": "#2c2d35",
	"dropdown.foreground": "#fdf6eb",
	"dropdown.border": "#29ccd0",
	"minimapGutter.addedBackground": "#bbc1c6",
	"minimapGutter.modifiedBackground": "#019ebb",
	"minimapGutter.deletedBackground": "#df0f60",
	"minimap.findMatchHighlight": "#019ebbd7",
	"minimap.selectionHighlight": "#264f78fe",
	"minimap.errorHighlight": "#f2064c",
	"minimap.warningHighlight": "#e18b81",
	"minimap.background": "#20272f",
	"sideBar.dropBackground": "#395a63",
	"editorGroup.emptyBackground": "#20272f",
	"panelSection.border": "#29ccd0a1",
	"statusBarItem.activeBackground": "#FFFFFF25",
	"settings.headerForeground": "#29ccd0",
	"settings.focusedRowBackground": "#ffffff07",
	"walkThrough.embeddedEditorBackground": "#00000050",
	"breadcrumb.activeSelectionForeground": "#d82a6d",
	"editorGutter.commentRangeForeground": "#fdf6eb",
	"debugExceptionWidget.background": "#303030",
	"debugExceptionWidget.border": "#29ccd0"
}};
	return theme
}

function registering(commandId , colourTrue , context) {
	const command = vscode.commands.registerCommand(commandId, async () => {
		const colour = await vscode.window.showInputBox({
			prompt: "Enter a hexadecimal value for the colour(6 charaters) or transparency(2 characters). For example : 5f2acc or 9d. The pre-entered value is the default value for the theme",
			value: defaultValues[commandId]
		});
		if (!colour || (colour.length != (colourTrue ? 6 : 2) || (/^[0-9a-fA-F]+$/.test(colour) == false))) {
			vscode.window.showInformationMessage("Nothing entered/cancelled command or incorrect format.");
			return;
		};
		
		const themePath = path.join(context.extensionPath, 'themes', 'Hatsune Miku-color-theme.json');
		
		fs.writeFileSync(themePath, JSON.stringify(themeConfig(commandId , colour , colourTrue), null, 2));

		await vscode.workspace.getConfiguration().update(
			'workbench.colorTheme',
			'Hatsune Miku',
			true
		);
		
		actDeact.intendedClose = true
		vscode.commands.executeCommand('workbench.action.reloadWindow');
	});
	context.subscriptions.push(command)
}

class actDeact {
	static intendedClose = false
	static viewSectionsDisplayed = JSON.parse(fs.readFileSync(path.join(__dirname, "intendedValuesSomeVariables.json"), 'utf8')).viewSectionsDisplayed
	static possibleCommandsToExecute = JSON.parse(fs.readFileSync(path.join(__dirname, "intendedValuesSomeVariables.json"), 'utf8')).possibleCommandsToExecute

	static activate(context) {

		vscode.commands.executeCommand('setContext', 'currentView', actDeact.viewSectionsDisplayed[actDeact.viewSectionsDisplayed.length - 1]);

		const changeDisplayedView = vscode.commands.registerCommand("changeDisplayedView", (WhichViewToDisplay) => {
			console.log(WhichViewToDisplay);
			if (WhichViewToDisplay[0] == "_") {
				var commandsToPush = WhichViewToDisplay.split('_').slice(1, -1);
				commandsToPush.length == 1 ? (actDeact.possibleCommandsToExecute.push(commandsToPush[0]), WhichViewToDisplay = "oneButton") : (actDeact.possibleCommandsToExecute.push(commandsToPush[0],commandsToPush[2]), WhichViewToDisplay = "twoButtons");
			}
			actDeact.viewSectionsDisplayed.push(WhichViewToDisplay);
			vscode.commands.executeCommand('setContext', 'currentView', WhichViewToDisplay);
		});
		context.subscriptions.push(changeDisplayedView);

		const returnPreviousDisplayedView = vscode.commands.registerCommand("returnPreviousDisplayedView", () => {
			actDeact.viewSectionsDisplayed.pop();
			vscode.commands.executeCommand('setContext', 'currentView', actDeact.viewSectionsDisplayed[actDeact.viewSectionsDisplayed.length - 1]);
		});
		context.subscriptions.push(returnPreviousDisplayedView)

		const executeFinalCommand = vscode.commands.registerCommand("executeFinalCommand", (whichIndex) => {
			if (whichIndex == "zero"){
				vscode.commands.executeCommand("changeThemeEditor." + actDeact.possibleCommandsToExecute[0]);
			}
			else {
				vscode.commands.executeCommand("changeThemeEditor." + actDeact.possibleCommandsToExecute[1]);
			}
		});
		context.subscriptions.push(executeFinalCommand)

		const returnPreviousDisplayedViewFinal = vscode.commands.registerCommand("returnPreviousDisplayedViewFinal", () => {
			actDeact.viewSectionsDisplayed.pop();
			vscode.commands.executeCommand('setContext', 'currentView', actDeact.viewSectionsDisplayed[actDeact.viewSectionsDisplayed.length - 1]);
			actDeact.possibleCommandsToExecute.length = 0;
		});
		context.subscriptions.push(returnPreviousDisplayedViewFinal)

		for (let i =0; i < packageJson.contributes.commands.length; i++) {
			const commandId = packageJson.contributes.commands[i].command
			if (commandId.slice(0,17) == "changeThemeEditor") {
				registering(commandId , commandId.slice(18,30) == "changecolour" , context);
			}
		}
	};
}


function activate (context) {
	actDeact.activate(context);
}

function deactivate () {
	if (actDeact.intendedClose == true) {
		fs.writeFileSync(path.join(__dirname, "intendedValuesSomeVariables.json"), JSON.stringify({"viewSectionsDisplayed": actDeact.viewSectionsDisplayed, "possibleCommandsToExecute": actDeact.possibleCommandsToExecute}, null, 2));
	}
	else {
		fs.writeFileSync(path.join(__dirname, "intendedValuesSomeVariables.json"), JSON.stringify({"viewSectionsDisplayed": ["A"], "possibleCommandsToExecute": []}, null, 2));
	}
}

module.exports = {
  activate,
  deactivate
};